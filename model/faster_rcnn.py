...
class FasterRCNN(nn.Module):
    ...
    def forward(self, x, scale=1.):
        ...
        Scaling paramter `scale` is used by RPN to determine the
        ...
        * `R'` is the total number of RoIs produced across batches. \
            Given `R_i` proposed RoIs from the `i` th image, \
            `R' = \\sum _{i=1} ^ N R_i`.
    ...
    def predict(self, imgs,sizes=None,visualize=False):
        ...
        Returns:
           ...
           * **bboxes**: A list of float arrays of shape `(R, 4)`, \
               where `R` is the number of bounding boxes in a image. \
               Each bouding box is organized by \
               `(y_{min}, x_{min}, y_{max}, x_{max})` \
               in the second axis.
           * **labels** : A list of integer arrays of shape `(R,)`. \
               Each value indicates the class of the bounding box. \
               Values are in range `[0, L - 1]`, where `L` is the \
               number of the foreground classes.
           * **scores** : A list of float arrays of shape `(R,)`. \
